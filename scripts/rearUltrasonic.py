...
class RearUltrasonic(object):

    def __init__(self):
        self.robot_host = re.sub("-", "_", socket.gethostname())
        self.ultrasonicPub = rospy.Publisher(self.robot_host + '/ultrasonic/rear/distance', Range, queue_size=10)
    ...
    def start(self):
        ...
        ultrasonic = UltrasonicParallax(21)
        ...
        while not rospy.is_shutdown():
            ...
            message_str = "rearUltrasonic Distance: %s cm and Speed: %s m/s" % (distance, relative_velocity)
...
if __name__ == '__main__':
    ...
    node_name = re.sub("-", "_", socket.gethostname()) + "_RearUltrasonic"
    rospy.init_node(node_name, anonymous=False)
    # Go to class functions that do all the heavy lifting.

    rear_ultrasonic = RearUltrasonic()

    try:
        rear_ultrasonic.start()
